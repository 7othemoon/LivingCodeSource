...
//连接的服务器域名，注意格式！！！
const host = 'wxs://3h222a6.mqtt.iot.gz.baidubce.com/mqtt';
Page({
  data: {
    client: null,
    //记录重连的次数
  ...
  onClick_PubMsg: function() {
    ...
    console.log("JSON.stringify(lightOpen):" + JSON.stringify(lightOpen))
    if (this.data.client && this.data.client.connected) {
      this.data.client.publish('/light/deviceIn', JSON.stringify(lightOpen), {qos:1});
      wx.showToast({
        title: '发布成功'
      })
    } else {
